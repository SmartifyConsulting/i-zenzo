...
function AuditLog() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-100">Audit &amp; usage</h1>
      <p className="mt-1 text-sm text-slate-400">Read-only audit trail of funder workspace actions.</p>

      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.02]">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="text-sm font-medium text-slate-100">Recent events</div>
        ...
        <div className="divide-y divide-white/5">
          ...
          {events.map((e) => (
            <div key={e.id} className="px-4 py-2.5 text-xs text-slate-300">
              <span className="text-slate-500">{new Date(e.created_at).toLocaleString()}</span>{" "}
              <span className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 text-[10px] capitalize text-slate-400">{e.event.replace(/_/g, " ")}</span>
