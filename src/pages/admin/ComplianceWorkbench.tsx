...
function Workbench() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-100">Compliance Case Management Workbench</h1>
      <p className="mt-1 text-sm text-slate-400">KYB / sanctions / EDD case queue.</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-white/10 bg-white/[0.03] p-4">
          ...
          <div className="mt-1 text-xl font-semibold text-slate-100">{cases.length}</div>
        </div>
      </div>
          ...
          {cases.map((c) => (
            <div key={c.id} className="flex items-center justify-between px-4 py-3 text-xs text-slate-300">
              <div>
                ...
                <div className="mt-0.5 text-slate-400">{c.subject}</div>
              ...
              <div className="flex gap-1.5">
                {c.status === "unassigned" && (
                  <button onClick={() => act(c, "in_progress", true)} disabled={busyId === c.id} className="rounded border border-white/10 px-2 py-1 text-[11px] text-slate-300 hover:bg-white/5 disabled:opacity-50">
                    Claim
                  </button>
                )}
                {c.status !== "resolved" && (
                  <button onClick={() => act(c, "resolved")} disabled={busyId === c.id} className="rounded border border-white/10 px-2 py-1 text-[11px] text-slate-300 hover:bg-white/5 disabled:opacity-50">
                    Resolve
                  </button>
                )}
              </div>
