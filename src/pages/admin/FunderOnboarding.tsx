...
function Requests() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-900">Onboarding requests</h1>
      <p className="mt-1 text-sm text-slate-500">Review, approve or reject funder onboarding requests.</p>

      <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <div className="text-sm font-medium text-slate-900">Requests</div>
          ...
          {requests.map((r) => (
            <div key={r.id} className="flex items-center justify-between px-4 py-3 text-xs text-slate-700">
              <div>
                ...
                <span className={`mt-1 inline-block rounded border px-1.5 py-0.5 text-[10px] font-medium capitalize ${r.status === "pending" ? "border-amber-500/30 bg-amber-500/10 text-amber-700" : r.status === "approved" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700" : "border-red-500/30 bg-red-500/10 text-red-700"}`}>
                  {r.status}
                </span>
              </div>
              {r.status === "pending" && (
