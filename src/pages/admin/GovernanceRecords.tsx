...
function Records() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-100">Governance Records</h1>
      ...
      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.02]">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="text-sm font-medium text-slate-100">Records ({records.length})</div>
        ...
        <div className="divide-y divide-white/5">
          ...
          {records.map((r) => (
            <div key={r.id} className="flex items-center justify-between px-4 py-3 text-xs text-slate-300">
              <div>
                <div className="font-mono text-[11px] text-slate-200">{r.id.slice(0, 8)}…</div>
                <div className="mt-0.5 text-slate-500">{new Date(r.created_at).toLocaleString()}</div>
              </div>
              <div className="flex gap-1.5 text-[10px]">
                <span className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 capitalize text-slate-400">{r.lifecycle}</span>
                <span className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 capitalize text-slate-500">{r.trading_stage}</span>
              </div>
            </div>
          ))}
        </div>
