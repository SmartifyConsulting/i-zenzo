...
function Repair() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-100">Legacy Repair</h1>
      <p className="mt-1 text-sm text-slate-400">Matches with conflicting status/state/POI fields. Hidden from user views.</p>

      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.02]">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="text-sm font-medium text-slate-100">{flags.length} match(es) flagged for repair</div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody>
              ...
              {flags.map((f) => (
                <tr key={f.id} className="border-b border-white/5 text-slate-300">
                  ...
                  <td className="px-4 py-2.5">
                    <div className="flex gap-1.5">
                      <button onClick={() => resolve(f, "archive")} disabled={busyId === f.id} className="rounded border border-white/10 px-2 py-1 text-[11px] text-slate-300 hover:bg-white/5 disabled:opacity-50">
                        Archive
                      </button>
                      <button onClick={() => resolve(f, "repair")} disabled={busyId === f.id} className="rounded border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-[11px] text-emerald-400 hover:bg-emerald-500/20 disabled:opacity-50">
                        Repair
                      </button>
                    </div>
                  </td>
