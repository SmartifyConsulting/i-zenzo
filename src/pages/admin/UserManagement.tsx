...
function UsersTable() {
  ...
  return (
    <>
      ...
      <p className="mt-1 text-sm text-slate-400">Profiles and role assignments across every workspace.</p>
      ...
      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.02]">
        <div className="flex items-center justify-between gap-3 border-b border-white/10 px-4 py-3">
          <input
            ...
            className="w-full max-w-xs rounded border border-white/10 bg-white/5 px-3 py-1.5 text-xs text-slate-200 placeholder:text-slate-600 focus:outline-none focus:ring-1 focus:ring-emerald-500"
          />
          <button
            onClick={load}
            disabled={loading}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody>
              ...
              {filtered.map((u) => (
                <tr key={u.user_id} className="border-b border-white/5 text-slate-300">
