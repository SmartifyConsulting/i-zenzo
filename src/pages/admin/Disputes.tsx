...
function ResolveForm({ dispute, onDone }: { dispute: DisputeRow; onDone: () => void }) {
  ...
  if (!open) {
    return (
      <button onClick={() => setOpen(true)} className="rounded border border-slate-200 px-2 py-1 text-[11px] text-slate-700 hover:bg-slate-50">
        Resolve
      </button>
    );
  }
  return (
    <div className="flex items-center gap-1.5">
      <input value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Resolution notes…" className="w-48 rounded border border-slate-200 bg-slate-100 px-2 py-1 text-[11px] text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500" />
      <button onClick={submit} disabled={saving || !notes.trim()} className="rounded bg-emerald-500/20 px-2 py-1 text-[11px] text-emerald-700 hover:bg-emerald-500/30 disabled:opacity-50">
        {saving ? "Saving…" : "Save"}
      </button>
    </div>
  );
...
function DisputeList() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-900">Dispute Resolution</h1>
      <p className="mt-1 text-sm text-slate-500">Flagged trades, escalations, force-resolve overrides.</p>
      ...
      <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-lg border border-slate-200 bg-slate-50 p-4">
          ...
          <div className="mt-1 text-xl font-semibold text-slate-900">{disputes.length}</div>
        </div>
      </div>

      <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <div className="text-sm font-medium text-slate-900">All Disputes</div>
              ...
              {disputes.map((d) => (
                <tr key={d.id} className="border-b border-slate-100 text-slate-700">
                  ...
                  <td className="px-4 py-2.5">
                    <span className={`rounded border px-1.5 py-0.5 text-[10px] font-medium capitalize ${d.status === "resolved" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700" : "border-amber-500/30 bg-amber-500/10 text-amber-700"}`}>
                      {d.status}
                    </span>
                  </td>
                  <td className="px-4 py-2.5 text-slate-500">{new Date(d.raised_at).toLocaleString()}</td>
                  <td className="px-4 py-2.5">
                    {d.status === "open" ? <ResolveForm dispute={d} onDone={load} /> : <span className="text-slate-400">—</span>}
                  </td>
                </tr>
              ))}
            </tbody>
