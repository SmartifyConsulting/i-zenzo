...
function UsageLog() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-100">API usage &amp; blocked events</h1>
      <p className="mt-1 text-sm text-slate-400">Safe audit-grade view. No raw payloads or full API keys are rendered.</p>

      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.02]">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="text-sm font-medium text-slate-100">{events.length} usage events · {blocked.length} blocked</div>
              ...
              {events.map((e) => (
                <tr key={e.id} className="border-b border-white/5 text-slate-300">
                  ...
                  <td className="px-4 py-2.5">
                    {e.blocked && <span className="mr-1 rounded border border-red-500/30 bg-red-500/10 px-1.5 py-0.5 text-[10px] text-red-400">blocked</span>}
                    {e.rate_limited && <span className="rounded border border-amber-500/30 bg-amber-500/10 px-1.5 py-0.5 text-[10px] text-amber-400">rate-limited</span>}
                  </td>
