...
function GovernanceList() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-900">P-5 Governance</h1>
      <p className="mt-1 text-sm text-slate-500">Governance, Compliance and Readiness, internal admin surface.</p>

      <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50">
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3">
          <div className="text-sm font-medium text-slate-900">Cases ({cases.length})</div>
          ...
          {cases.map((c) => (
            <div key={c.id} className="flex items-center justify-between px-4 py-3 text-xs text-slate-700">
              <div>
                ...
                <div className="mt-0.5 text-slate-500">{c.summary}</div>
                <div className="mt-1 flex gap-2 text-[10px]">
                  <span className="rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 capitalize text-slate-500">{c.status.replace(/_/g, " ")}</span>
                  <span className="rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 capitalize text-slate-500">{c.category.replace(/_/g, " ")}</span>
                  {c.transaction_id && (
                    <span className="rounded border border-slate-200 bg-slate-100 px-1.5 py-0.5 font-mono text-slate-400">match {c.transaction_id.slice(0, 8)}…</span>
                  )}
                </div>
              </div>
              {!c.assigned_to && (
                <button onClick={() => claim(c)} disabled={busyId === c.id} className="rounded border border-slate-200 px-2 py-1 text-[11px] text-slate-700 hover:bg-slate-50 disabled:opacity-50">
                  {busyId === c.id ? "Working…" : "Claim"}
                </button>
              )}
              {c.assigned_to && <span className="text-[11px] text-slate-400">Assigned</span>}
            </div>
          ))}
        </div>
      </div>
