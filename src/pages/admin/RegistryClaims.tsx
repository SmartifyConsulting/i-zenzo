...
function ClaimQueue() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-100">Claim queue</h1>
      <p className="mt-1 text-sm text-slate-400">Company ownership claims awaiting review.</p>

      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.02]">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="text-sm font-medium text-slate-100">Claims ({claims.length})</div>
        ...
        <div className="divide-y divide-white/5">
          ...
          {claims.map((c) => (
            <div key={c.id} className="flex items-center justify-between px-4 py-3 text-xs text-slate-300">
              <div>
                <div className="text-slate-100">{c.company_name}</div>
                <div className="mt-0.5 text-slate-500">{c.claimant_email}</div>
                <span className={`mt-1 inline-block rounded border px-1.5 py-0.5 text-[10px] font-medium capitalize ${c.status === "pending" ? "border-amber-500/30 bg-amber-500/10 text-amber-400" : c.status === "approved" ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-400" : "border-red-500/30 bg-red-500/10 text-red-400"}`}>
                  {c.status}
                </span>
              </div>
              {c.status === "pending" && (
                <div className="flex gap-1.5">
                  <button onClick={() => decide(c, true)} disabled={busyId === c.id} className="rounded border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-[11px] text-emerald-400 hover:bg-emerald-500/20 disabled:opacity-50">
                    Approve
                  </button>
                  <button onClick={() => decide(c, false)} disabled={busyId === c.id} className="rounded border border-red-500/30 bg-red-500/10 px-2 py-1 text-[11px] text-red-400 hover:bg-red-500/20 disabled:opacity-50">
                    Reject
                  </button>
                </div>
              )}
