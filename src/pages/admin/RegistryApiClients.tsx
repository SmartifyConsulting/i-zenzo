...
function ClientsTable() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-100">Institutional API clients</h1>
      <p className="mt-1 text-sm text-slate-400">Read-only safe summary. No secrets, no full keys are rendered.</p>

      <div className="mt-6 rounded-lg border border-white/10 bg-white/[0.02]">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div className="text-sm font-medium text-slate-100">{clients.length} clients</div>
            ...
            <div key={c.id} className="flex items-center justify-between px-4 py-3 text-xs text-slate-300">
              <div>
                ...
                <div className="mt-1 flex gap-2 text-[10px]">
                  <span className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 capitalize text-slate-400">{c.lifecycle_status.replace(/_/g, " ")}</span>
                  <span className="rounded border border-white/10 bg-white/5 px-1.5 py-0.5 capitalize text-slate-500">{c.mode}</span>
                  {c.country && <span className="text-slate-600">{c.country}</span>}
                </div>
              </div>
              <div className="flex gap-1.5">
                {c.lifecycle_status !== "production_active" && (
                  <button onClick={() => setStatus(c, "production_active")} disabled={busyId === c.id} className="rounded border border-emerald-500/30 bg-emerald-500/10 px-2 py-1 text-[11px] text-emerald-400 hover:bg-emerald-500/20 disabled:opacity-50">
                    Activate
                  </button>
                )}
                {c.lifecycle_status !== "suspended" && (
                  <button onClick={() => setStatus(c, "suspended")} disabled={busyId === c.id} className="rounded border border-red-500/30 bg-red-500/10 px-2 py-1 text-[11px] text-red-400 hover:bg-red-500/20 disabled:opacity-50">
                    Suspend
                  </button>
                )}
              </div>
