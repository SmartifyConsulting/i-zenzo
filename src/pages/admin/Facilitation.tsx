...
function CaseQueue() {
  ...
  return (
    <>
      ...
      <h1 className="text-2xl font-semibold text-slate-900">Facilitation</h1>
              ...
              {data?.cases.map((c: CaseRow) => {
                ...
                return (
                  <tr key={c.id} className="border-b border-slate-100 text-slate-700">
                    <td className="px-4 py-2.5 font-mono text-[11px] text-slate-800">{c.case_number}</td>
                    ...
                    <td className="px-4 py-2.5">
                      ...
                      {c.owner_id && <span className="text-slate-400">Assigned</span>}
                    </td>
                  </tr>
                );
              })}
  ...
  return (
    <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50">
      <div className="border-b border-slate-200 px-4 py-3">
        <div className="text-sm font-medium text-slate-900">Outreach email templates</div>
          ...
          <div key={t.id} className="px-4 py-3">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                ...
                <p className="mt-1 max-w-2xl whitespace-pre-line text-[11px] text-slate-500">{t.body}</p>
              </div>
              <button
                onClick={() => toggleApprove(t)}
                disabled={busyId === t.id}
                className="shrink-0 rounded border border-slate-200 px-2 py-1 text-[11px] text-slate-700 hover:bg-slate-50 disabled:opacity-50"
              >
...
function DncRules() {
  ...
  return (
    <div className="mt-6 rounded-lg border border-slate-200 bg-slate-50">
      <div className="border-b border-slate-200 px-4 py-3">
        <div className="text-sm font-medium text-slate-900">Do-not-contact rules</div>
      ...
      <div className="flex flex-wrap items-end gap-2 border-b border-slate-200 px-4 py-3">
        ...
        <input
          ...
          placeholder="Value…"
          className="w-48 rounded border border-slate-200 bg-slate-100 px-2 py-1.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"
        />
        <input
          value={reason}
          onChange={(e) => setReason(e.target.value)}
          placeholder="Reason (optional)…"
          className="w-56 rounded border border-slate-200 bg-slate-100 px-2 py-1.5 text-xs text-slate-800 placeholder:text-slate-400 focus:outline-none focus:ring-1 focus:ring-emerald-500"
        />
        <button
          onClick={addRule}
      ...
      <div className="divide-y divide-slate-100">
        ...
        {rules.map((r) => (
          <div key={r.id} className="flex items-center justify-between px-4 py-2.5 text-xs text-slate-700">
            <div>
